...
// #include <cxd56_i2c.h>
// #include <arch/arm/src/cxd56xx/cxd56_i2c.h>
...
int VL53L1X_Init(struct VL53L1X_dev_s *device, uint8_t address)
{
    ...
    status = VL53L1X_SetI2CAddress(device, address*2);
    if (status < 0)
    {
        _info("Error occurred while setting new I2C address.\n");
        return status;
    }
...
int VL53L1X_WrByte(struct VL53L1X_dev_s *device, uint16_t index, uint8_t data)
{
    ...
    // Need to write first the slave address.
    uint8_t address[2];
    address[0] = (uint8_t)(index >> 8);
    address[1] = (uint8_t)(index & 0xFF);

    // First write out the address of the register we want
    ret = i2c_write(device->_dev, &config, address, 2);
    if (ret < 0) {
        _info("Error occurred while writing address on bus.\n");
        return ret;
    }

    struct i2c_config_s config2;
    config2.address = index;
    config2.addrlen = 7;
    config2.frequency = device->_freq;

    // Then go ahead and write out the data we pass.
    ret = i2c_write(device->_dev, &config2, &data, 1);
    if (ret < 0) {
        _info("Error occurred while writing new address on bus.\n");    
    }
    return ret;
...
uint16_t VL53L1X_RdWord(struct VL53L1X_dev_s *device, uint16_t index, uint16_t *data)
{
    ...
    status = VL53L1X_I2CRead(device, index, buffer, 2);
    if (!status)
    {
...
static int VL53L1X_open(FAR struct file *filep)
{
    ...
    ret = VL53L1X_Init(priv, priv->_i2c_address);
    if (ret < 0) {
        _info("Error occurred while initializing VL53L1X sensor.\n");
        return ERROR;
    }
    _info("Device is available.\n");
    return OK;
}
